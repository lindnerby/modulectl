...
var _ = Describe("Test 'create' Command", Ordered, func() {
	...
	Context("Given a directory with an existing module configuration file", func() {
		BeforeAll(func() {
			setup()
			Expect(createMarkerFile("create-module-config.yaml")).To(Succeed())
		})
		AfterAll(func() { teardown() })
		...
		It("Then the command should fail", func() {
			...
			Expect(filesIn(workDir)).Should(ContainElement("create-module-config.yaml"))
			Expect(getMarkerFileData("create-module-config.yaml")).Should(Equal(markerFileData))
		})
	})
	...
	Context("Given a directory with an existing module configuration file", func() {
		...
		var cmd createCmd
		It("When `modulectl create` command is invoked with --overwrite flag", func() {
			cmd = createCmd{
				overwrite: true,
			}
		})
	...
	Context("Given an empty directory", func() {
		...
		var cmd createCmd
		It("When `modulectl create` command args override defaults", func() {
			cmd = createCmd{
	...
	Context("Given a directory with existing files", func() {
		...
		var cmd createCmd
		It("When `modulectl create` command is invoked with arguments that match existing files names", func() {
			cmd = createCmd{
				genManifestFlag:               "custom-manifest.yaml",
				genDefaultCRFlag:              "custom-default-cr.yaml",
				genSecurityScannersConfigFlag: "custom-security-scanners-config.yaml",
...
func (cmd *createCmd) toConfigBuilder() *moduleConfigBuilder {
...
// This is a copy of the moduleConfig struct from internal/create/contentprovider/moduleconfig.go
